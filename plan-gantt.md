...
function StoryBar({ data }: { data: ITask }) {
  return (
    <div className="flex items-center gap-1 px-1 h-full text-white text-xs">
      ...
      {data.storyPoints && <span className="ml-auto opacity-70">{data.storyPoints}sp</span>}
    </div>
  )
}
```
...
*Foundational features that make the Gantt actually useful for planning.*
...
*Features that differentiate us from a basic chart.*
...
*What makes this tool special vs. generic alternatives.*
...
*Intelligence layer that makes scheduling proactive rather than reactive.*
...
*Nice-to-haves that make the experience feel premium.*
...
*This document is a living wishlist. Features should be pulled from these tiers based on user needs, not implemented top-to-bottom.*
